...
function ossn_generate_action_token_robot($timestamp){
    if(!isset($timestamp) && empty($timestamp)){
        $timestamp = time();
    }
    $site_screat = ossn_site_settings('site_key');
    $session_id = session_id();
    $user_guid  = ossn_robot_uid()->guid;

    if(!$user_guid) {
        ossn_trigger_message(ossn_print('robot:account:fail'), 'error');
        redirect(REF);
    }
...
function ossn_add_tokens_to_url_robot($url){
    $params = parse_url($url);
    ...
    $tokens['ossn_ts'] = time();
    $tokens['ossn_token'] = ossn_generate_action_token_robot($tokens['ossn_ts']);
    $tokens = array_merge($query, $tokens);
